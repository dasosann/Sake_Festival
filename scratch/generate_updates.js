// Complete booth data from user - all 143 entries
const completeData = `1,㈜니혼슈코리아 판매부스,A01,,
2,㈜니혼슈코리아 안내부스,A02,,
...
lines.forEach(line => {
    ...
    if (parts.length >= 3) {
        ...
        boothUpdates.set(boothId, { name, region, products });
    }
});

// Generate TypeScript replacement text
let tsOutput = '// A section updates needed:\n';
for (const [boothId, data] of boothUpdates) {
    if (boothId.startsWith('A')) {
        tsOutput += `\n${boothId}: ${data.name}\n`;
        if (data.region) tsOutput += `  Region: ${data.region}\n`;
        if (data.products.length > 0) {
            tsOutput += `  Products: ${data.products.length} items\n`;
            tsOutput += `    ${data.products.slice(0, 2).join(', ')}${data.products.length > 2 ? '...' : ''}\n`;
        }
    }
}

console.log(tsOutput);
console.log('\nTotal booths to update:', boothUpdates.size);
