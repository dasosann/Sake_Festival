...
const ProductItem = ({
  ...
  return (
    <div className="bg-white border border-glass-border rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-md hover:border-primary/20">
      <div className="p-4 flex gap-4">
        <div className="w-20 h-28 bg-bg-sub rounded-xl shrink-0 flex items-center justify-center overflow-hidden border border-glass-border">
          {product.image ? (
            ...
            <span className="text-[1.5rem] opacity-30 grayscale hover:grayscale-0 hover:opacity-100 transition-all duration-500 scale-100 hover:scale-110">
              🍶
            </span>
          )}
        </div>
        <div className="flex-1 flex flex-col justify-between">
          <div className="flex justify-between items-start mb-2">
            ...
            <button
              onClick={() => toggleFavoriteProduct(boothId, idx)}
              className={`shrink-0 transition-transform active:scale-90 ${favoriteProducts.includes(`${boothId}_${idx}`) ? "text-red-500" : "text-text-dim/20 hover:text-red-300"}`}
            >
              <HeartIcon
                filled={favoriteProducts.includes(`${boothId}_${idx}`)}
                className="w-4 h-4"
              />
            </button>
          </div>
          <div className="relative">
            <textarea
              ...
              value={productNotes[`${boothId}_${idx}`] || ""}
              onChange={(e) => updateProductNote(boothId, idx, e.target.value)}
          ...
          <div className="space-y-3">
            {product.abv && (
              <div className="flex items-center gap-2">
                <span className="text-[0.6rem] font-black px-2 py-0.5 bg-primary/10 text-primary rounded-full">
                  도수 {product.abv}
                </span>
              </div>
            )}
  ...
  return (
    ...
    >
      ...
      >
        {/* Modal Header */}
        <div
          className={`p-3 ${getBoothColorClass(selectedBooth.id.charAt(0))} border-b`}
        >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <div className="flex items-center justify-between p-4 rounded-2xl bg-bg-sub border border-glass-border">
            <div className="flex items-center gap-3">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${visitedBooths.includes(selectedBooth.id) ? "bg-primary text-white shadow-lg" : "bg-white border border-glass-border text-text-dim/40"}`}
              >
                {visitedBooths.includes(selectedBooth.id) ? "✓" : "🍷"}
              </div>
              <div>
                <p className="text-[0.6rem] font-bold text-text-dim uppercase tracking-wider leading-none mb-1">
                  방문 상태
                </p>
                <p
                  className={`text-[0.8rem] font-black ${visitedBooths.includes(selectedBooth.id) ? "text-primary" : "text-text"}`}
                >
                  {visitedBooths.includes(selectedBooth.id)
                    ? "방문 완료"
                    : "미방문"}
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <HeartIcon
                  filled={favoriteBooths.includes(selectedBooth.id)}
                  className="w-6 h-6"
                />
              </button>
              <button
            ...
            <>
              ...
              <section className="bg-primary/5 p-5 rounded-2xl border border-primary/10">
                <h4 className="text-[0.7rem] font-black text-primary uppercase tracking-widest mb-2 flex items-center gap-2">
                  <span className="w-5 h-5 rounded-md bg-white flex items-center justify-center shadow-sm">
                    📜
                  </span>
                ...
                {selectedBooth.details.brewery && (
                  <p className="text-[0.7rem] leading-relaxed text-text-dim font-bold mb-2 whitespace-pre-wrap">
                    양조장: {selectedBooth.details.brewery}
                  </p>
                )}
                <p className="text-[0.8rem] leading-relaxed text-text font-medium opacity-90 whitespace-pre-wrap">
                  {selectedBooth.details.history}
                </p>
              </section>
              ...
              <section className="bg-bg p-5 rounded-2xl border border-glass-border">
                <h4 className="text-[0.7rem] font-black text-text uppercase tracking-widest mb-2 flex items-center gap-2">
                  <span className="w-5 h-5 rounded-md bg-bg-sub flex items-center justify-center shadow-sm">
                    ✨
                  </span>
                  {["food", "service"].includes(
                    selectedBooth.details.type || "",
                ...
                <p className="text-[0.8rem] leading-relaxed text-text font-medium opacity-90 whitespace-pre-wrap">
                  {selectedBooth.details.features}
                </p>
              </section>

              {/* 사케 출품 목록 (Exhibited Products) - Moved Up */}
              {selectedBooth.details.products &&
                selectedBooth.details.products.length > 0 && (
                  <section className="pt-6 border-t border-glass-border">
                    <div className="flex items-center justify-between mb-4">
                      <h4 className="text-[0.7rem] font-black text-primary uppercase tracking-widest flex items-center gap-2">
                        <span className="w-5 h-5 rounded-md bg-primary/10 text-primary flex items-center justify-center shadow-sm text-[0.8rem]">
                          🍶
                        </span>
                        사케 출품 목록
                      </h4>
              ...
              <section className="pt-6 border-t border-glass-border">
                <h4 className="text-[0.7rem] font-black text-text-dim uppercase tracking-widest mb-4 flex items-center gap-2">
                  <span className="w-5 h-5 rounded-md bg-bg-sub flex items-center justify-center shadow-sm text-[0.8rem]">
                    ✍️
                  </span>
                  나의 테이스팅 노트
                </h4>
                ...
                <div className="flex justify-between items-center mt-2 px-1">
                  <span className="text-[0.6rem] text-text-dim/60 font-medium italic">
                    입력 시 실시간 자동 저장됩니다
                  </span>
                  <span className="text-[0.6rem] text-text-dim/60 font-medium">
                    {(boothNotes[selectedBooth.id] || "").length}자
                  </span>
                </div>
              </section>
