    ...
    day23: [
      {
        time: "11:00",
        title: "축제 개막 (Day 1)",
        desc: "2026 서울 사케 페스티벌의 첫 번째 날이 시작됩니다.",
      },
      {
        time: "13:00 ~ 13:30",
        title: "닷사이 블루 마스터 클래스: Dassai Blue의 도전",
        desc: "금액: 15,000원 / 진행: 닷사이 대표이사 사쿠라이 카즈히로",
      },
      {
        time: "14:30 ~ 15:30",
        title: "닛카 위스키 마스터 클래스",
        desc: "금액: 10,000원 / 진행: 닛카위스키 Regional Manager 코키 키타무라",
      },
      {
        time: "16:00 ~ 16:30",
        title: "사케 톡톡 퀴즈쇼",
        desc: "진행: 2025 Miss SAKE JAPAN 그랑프리 타테노 치사토, 2025 MISS SAKE 아키타 시마미야 리사",
      },
      {
        time: "18:00",
        title: "Day 1 마감",
        desc: "첫째 날 모든 부스 운영이 종료됩니다.",
      },
    ],
    day24: [
      {
        time: "11:00",
        title: "축제 개장 (Day 2)",
        desc: "2026 서울 사케 페스티벌의 두 번째 날이 시작됩니다.",
      },
      {
        time: "13:30 ~ 14:30",
        title: "여성 쿠라모토들이 말하는 시대의 변화",
        desc: "[사회] 사카쇼 / 니혼슈 탄바의 타치바나 유키\n[패널] 이마다주조혼텐의 이마다 미호, 하야시혼텐의 하야시 리에코, 키리시마쵸증류소의 후루야 아키코",
      },
      {
        time: "15:00 ~ 15:30",
        title: "사케 톡톡 퀴즈쇼",
      ...
      },
      {
        time: "17:00",
        title: "축제 폐막",
        desc: "2026 서울 사케 페스티벌의 대단원이 막을 내립니다.",
      },
    ],
  };
      ...
      {(activeSubTab === "day23" || activeSubTab === "day24") && (
        <div className="glass-card divide-y divide-glass-border overflow-hidden">
          {dailySchedule[activeSubTab].map((item, index) => (
            <div
              key={index}
              className="flex gap-6 sm:gap-10 p-5 sm:p-6 hover:bg-bg-sub transition-colors duration-200"
            >
              <div className="font-black text-primary text-lg min-w-[70px] tabular-nums">
                {item.time}
              </div>
              <div className="flex flex-col gap-1">
                <h4 className="text-base font-bold text-text">{item.title}</h4>
                <p className="text-sm text-text-dim leading-relaxed font-medium whitespace-pre-line">
                  {item.desc}
                </p>
              </div>
            </div>
      ...
      {activeSubTab === "tasting" && (
        <div className="space-y-12">
          {premiumTastings.map((tasting, idx) => (
            <div
              key={idx}
              className={`rounded-3xl p-6 sm:p-8 ${tasting.theme} border border-glass-border shadow-sm`}
            >
              <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-8">
                <div className="flex flex-col gap-2">
                  ...
                  <p className="text-sm text-text-dim font-medium max-w-xl leading-relaxed">
                    {tasting.desc}
                  </p>
                </div>
                  ...
                  <div key={i} className="flex flex-col gap-4 group">
                    <div className="aspect-[3/4] rounded-xl sm:rounded-2xl bg-white shadow-sm overflow-hidden border border-glass-border group-hover:shadow-md transition-all">
                      <img
                        ...
                        onError={(e) => {
                          e.currentTarget.src =
                            "https://placehold.co/400x600/f3f4f6/9ca3af?text=Bottle";
                        }}
                      />
                    </div>
          ...
          <div className="glass-card p-6 sm:p-8 bg-bg-sub/30 border-dashed border-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                {
                  id: 1,
                  text: "시음권 구매를 위해 서울 사케 페스티벌 입장권이 필요합니다.",
                },
                { id: 2, text: "1인당 종류별로 1매씩만 구매 가능합니다." },
                {
                  id: 3,
                  text: "예약권은 조기에 매진될 수 있으니 미리 확인해 주세요.",
                },
              ].map((rule) => (
                <div
                  key={rule.id}
                  className="flex items-start gap-3 bg-white/50 p-4 rounded-2xl border border-glass-border"
                >
